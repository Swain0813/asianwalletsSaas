...
public class FundChangeFailMQReceive {
    ...
    public void processOrderFundChangeFail(String value) {
        ...
        if (rabbitMassage.getCount() > 0) {
            ...
            messageFeign.sendSimple(mobile, "SAAS-下单支付成功时上报清结算失败 MQ_PLACE_ORDER_FUND_CHANGE_FAIL 预警 :{ " + value + " }");
            messageFeign.sendSimpleMail(email, "SAAS-下单支付成功时上报清结算失败 MQ_PLACE_ORDER_FUND_CHANGE_FAIL 预警", "MQ_PLACE_ORDER_FUND_CHANGE_FAIL 预警 ：{ " + value + " }");
        }
    }
}
