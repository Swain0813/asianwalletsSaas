...
@Component
@Slf4j
public class MegaPayQueryMQReceive {
    ...
    public void megaPayQuery(String value) {
        ...
        if (rabbitMassage.getCount() > 0) {
            ...
            log.info("==============【MegaPay-THB查询队列1】============== value: {}", value);
            ...
            log.info("==============【MegaPay-THB查询队列1】==============【Channels服务MegaPay-THB接口响应参数】 response: {}", JSON.toJSONString(response));
            if (!TradeConstant.HTTP_SUCCESS.equals(response.getCode())) {
                log.info("==============【MegaPay-THB查询队列1】==============【接口响应错误 继续上报MegaPay-THB查询订单队列1】");
            ...
            if (orders == null) {
                log.info("==============【MegaPay-THB查询队列1】==============【查询订单信息不存在】 orderId :{}", orderId);
                return;
            }
            //订单已支付
            if (!orders.getTradeStatus().equals(TradeConstant.ORDER_PAYING)) {
                log.info("==============【MegaPay-THB查询队列1】==============【订单状态不为支付中】");
                return;
            }
            //订单状态
            ...
            if ("1".equals(result)) {
                log.info("==============【MegaPay-THB查询队列1】============== 【订单已支付成功】 orderID : {}", orders.getId());
                //未发货
                ...
                try {
                    ...
                    log.error("==============【MegaPay-THB查询队列1】============== 【更新通道订单异常】", e);
                }
                //修改订单状态
                int i = ordersMapper.updateByExampleSelective(orders, example);
                if (i > 0) {
                    log.info("==============【MegaPay-THB查询队列1】============== 【订单支付成功后更新数据库成功】 orderId: {}", orders.getId());
                    //计算支付成功时的通道网关手续费
                    ...
                    log.info("==============【MegaPay-THB查询队列1】============== 【订单支付成功后更新数据库失败】 orderId: {}", orders.getId());
                }
            } else if ("9".equals(result)) {
                log.info("==============【MegaPay-THB查询队列1】============== 订单已支付失败 orderID : {}", orders.getId());
                //支付失败
                ...
                try {
                    ...
                    log.info("==============【MegaPay-THB查询队列1】============== 【更新通道订单异常】", e);
                }
                //计算支付失败时通道网关手续费
                ...
                if (ordersMapper.updateByExampleSelective(orders, example) == 1) {
                    ...
                    log.info("==============【MegaPay-THB查询队列1】============== 【订单支付失败后更新数据库失败】 orderId: {}", orders.getId());
                }
            } else {
                log.info("==============【MegaPay-THB查询队列1】============== 【订单是交易中】 orderID : {}", orders.getId());
                log.info("==============【MegaPay-THB查询队列1】============== 【继续上报查询订单队列1】 【E_MQ_MEGAPAY_THB_CHECK_ORDER】");
                rabbitMQSender.send(AD3MQConstant.E_MQ_MEGAPAY_THB_CHECK_ORDER, JSON.toJSONString(rabbitMassage));
                return;
            }
            //回调商户
            ...
            log.info("==============【MegaPay-THB查询队列1】============== 【三次查询,订单为交易中】 rabbitMassage : {}", JSON.toJSONString(rabbitMassage));
            RabbitMassage rabbitMassage2 = new RabbitMassage(AsianWalletConstant.TWO, rabbitMassage.getValue());
            rabbitMQSender.send(AD3MQConstant.E_MQ_MEGAPAY_THB_CHECK_ORDER2, JSON.toJSONString(rabbitMassage2));
            log.info("==============【MegaPay-THB查询队列1】============== 【继续发送到MegaPay-THB查询队列2】");
        }
    }
