...
public class NganLuongMQRecive {
    ...
    public void processNganLuongCheckOrder(String value) {
        ...
        if (rabbitMassage.getCount() > 0) {
            ...
            log.info("==============【NL查询队列1】==============【查询参数记录】 map:{}", JSON.toJSONString(map));
            ...
            log.info("==============【NL查询队列1】============== http状态码: {},body: {}", status, JSON.toJSONString(body));
            //判断HTTP状态码
            if (status != AsianWalletConstant.HTTP_SUCCESS_STATUS) {
                log.info("==============【NL查询队列1】==============【状态码异常】");
                return;
            }
            // 注解方式xml转换为map对象
            if (StringUtils.isEmpty(body)) {
                log.info("==============【NL查询队列1】==============【调用查询接口返回body为空】");
                return;
            }
            Map<String, String> resultMap = null;
            try {
                resultMap = XMLUtil.xmlToMap(body, "UTF-8");
                log.info("==============【NL查询队列1】==============【解析后的XML结果】 resultMap:{}", JSON.toJSONString(resultMap));
            } catch (Exception e) {
                log.error("==============【NL查询队列1】==============【xml转换异常】", e);
            }
            ...
            if (StringUtils.isEmpty(transactionStatus) || !"00".equals(errorCode)) {
                log.info("==============【NL查询队列1】==============【回调参数有错误 继续上报NL查询订单队列1 E_MQ_NGANLUONG_CHECK_ORDER_DL】==========");
            ...
            if (orders == null) {
                log.info("==============【NL查询队列1】==============【查询订单信息不存在】 orderCode :{}", orderCode);
                return;
            }
            //订单已支付
            if (!orders.getTradeStatus().equals(TradeConstant.ORDER_PAYING)) {
                log.info("==============【NL查询队列1】==============【订单状态不为支付中】");
                return;
            }
            ...
            if ("00".equals(transactionStatus)) {
                log.info("==============【NL查询队列1】============== 【订单已支付成功】 orderID : {}", orders.getId());
                //支付成功
                ...
                try {
                    ...
                    log.error("==============【NL查询队列1】============== 【更新通道订单异常】", e);
                }
                //修改订单状态
                int i = ordersMapper.updateByExampleSelective(orders, example);
                if (i > 0) {
                    log.info("==============【NL查询队列1】============== 【订单支付成功后更新数据库成功】 orderId: {}", orders.getId());
                    //计算支付成功时的通道网关手续费
                    ...
                    try {
                        ...
                        if (commonRedisDataService.getAccountByMerchantIdAndCurrency(orders.getMerchantId(), orders.getOrderCurrency()) == null) {
                            log.info("==============【NL查询队列1】============== 【上报清结算前线下下单创建账户信息】");
                    ...
                    log.info("==============【NL查询队列1】============== 【订单支付成功后更新数据库失败】 orderId: {}", orders.getId());
                }
            } else if ("02".equals(transactionStatus)) {
                log.info("==============【NL查询队列1】============== 订单已支付失败 orderID : {}", orders.getId());
                //支付失败
                ...
                if (ordersMapper.updateByExampleSelective(orders, example) == 1) {
                    ...
                    log.error("==============【NL查询队列1】============== 【订单支付失败后更新数据库失败】 orderId: {}", orders.getId());
                }
            } else {
                log.info("==============【NL查询队列1】============== 【订单是交易中】 orderID : {}", orders.getId());
                log.info("==============【NL查询队列1】============== 【继续上报查询订单队列1】 【E_MQ_NGANLUONG_CHECK_ORDER_DL】  token: {}", token);
                rabbitMQSender.send(AD3MQConstant.E_MQ_NGANLUONG_CHECK_ORDER_DL, JSON.toJSONString(rabbitMassage));
                return;
            }
            //回调商户
            try {
                ...
                log.info("==============【NL查询队列1】==============【回调商户异常】", e);
            }
        } else {
            log.info("==============【NL查询队列1】============== 【三次查询,订单为交易中】 rabbitMassage : {}", JSON.toJSONString(rabbitMassage));
            RabbitMassage rabbitMassage2 = new RabbitMassage(AsianWalletConstant.TWO, rabbitMassage.getValue());
            rabbitMQSender.send(AD3MQConstant.E_MQ_NGANLUONG_CHECK_ORDER_DL2, JSON.toJSONString(rabbitMassage2));
            log.info("==============【NL查询队列1】============== 【继续发送到NL查询队列2】 rabbitMassage2 : {}", JSON.toJSONString(rabbitMassage2));
        }
    }
}
