...
public class ClearingServiceImpl implements ClearingService {
    ...
    public BaseResponse fundChange(FundChangeDTO fundChangeDTO) {
        log.info("------------交易服务  上报清结算 资金变动接口 ------------ fundChangeDTO : {} ", JSON.toJSON(fundChangeDTO));
        ...
        try {
            ...
            log.info("------------交易服务 上报清结算 资金变动接口 返回 ------------ fundChangeVO : {} ", JSON.toJSON(fundChangeVO));
            ...
            log.info("************交易服务 资金变动接口发生异常************** e:{}", e.getMessage());
            baseResponse.setCode("T001");
        }
        log.info("------------ 交易服务 上报清结算 返回 fundChange ------------ BaseResponse : {}", JSON.toJSON(baseResponse));
        return baseResponse;
    }
    ...
    public BaseResponse freezingFunds(FinancialFreezeDTO financialFreezeDTO) {
        log.info("------------交易服务 上报清结算 资金冻结接口 ------------ financialFreezeDTO : {} ", JSON.toJSON(financialFreezeDTO));
        BaseResponse baseResponse = new BaseResponse();
        try {
            //调用资金冻结接口生成签名
            ...
            log.info("------------交易服务 上报清结算 资金冻结接口 返回 ------------ financialFreezeVO : {} ", JSON.toJSON(financialFreezeVO));
            ...
            log.info("*************交易服务 资金冻结接口发生异常**************", e.getMessage());
            baseResponse.setCode("T001");
        }
