...
import com.asianwallets.common.dto.th.exception.MacException;
...
public class MACUtil {
    ...
    public static String getX919Mac(String mak, String macData, int filleType) throws MacException {
        try {
            ...
            byte[] byteMak = ISOUtil.hexStringToByte(mak.trim());
...
//				throw new MacException("Fill type expected {" + FILL_0X00 + "," + FILL_0X80 + "} [" + filleType + "]");
            ...
            // 明文不足8字节倍数时补足位数
            macData = ISOUtil.fillMac(macData);

            // 分段截取8字节，并进行异或/加密 (注：ASCII明文16位十六进制即为8字节)
            byte[] xorSrc = ISOUtil.hexStringToByte("0000000000000000");    // 初始异或值为8字节0
            for (int i = 0; i < macData.length() / 16; i++) {
                // 截取8字节数据
                String strXor = macData.substring(i * 16, i * 16 + 16);
                byte[] xorDest = ISOUtil.hexStringToByte(strXor);
            ...
            byteMac = ISOUtil.bytesToHexString(byteMac).getBytes();

            // 将十六进制数组转换为字符串
            return ISOUtil.bytesToHexString(byteMac);
        } catch (Exception e) {
    ...
    public static String getDoubleCupEcbMac(String mak, String macBlock) throws MacException {
        try {
            ...
            // 明文不足8字节倍数时补足位数
            macBlock = ISOUtil.fillMac(macBlock);
            ...
            byte[] xorResult = ISOUtil.bytesToHexString(xorSrc).getBytes();
    ...
    public static String getHBCCEcbMac(String mak, String macBlock) throws MacException {
        try {
            ...
            String strHexMac = ISOUtil.bytesToHexString(xorResult) + "8000000000000000";
