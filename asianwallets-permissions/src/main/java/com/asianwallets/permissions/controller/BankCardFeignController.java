...
public class BankCardFeignController extends BaseController {
    ...
    public BaseResponse addBankCard(@RequestBody @ApiParam List<BankCardDTO> bankCardDTO) {
        operationLogService.addOperationLog(this.setOperationLog(this.getUserName(), AsianWalletConstant.ADD, JSONArray.toJSONString(bankCardDTO),
                "添加银行卡信息"));
    ...
    public BaseResponse selectBankCardByMerId(@RequestParam @ApiParam String merchantId) {
        operationLogService.addOperationLog(this.setOperationLog(this.getUserName(), AsianWalletConstant.SELECT, JSONObject.toJSONString(this.getRequest().getParameterMap()),
                "根据机构id查询银行卡"));
    ...
    public BaseResponse pageBankCard(@RequestBody @ApiParam BankCardSearchDTO bankCardSearchDTO) {
        operationLogService.addOperationLog(this.setOperationLog(this.getUserName(), AsianWalletConstant.SELECT, JSONArray.toJSONString(bankCardSearchDTO),
                "分页查询银行卡"));
