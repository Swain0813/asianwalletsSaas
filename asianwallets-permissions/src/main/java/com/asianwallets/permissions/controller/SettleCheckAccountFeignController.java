...
public class SettleCheckAccountFeignController extends BaseController {
    ...
    public BaseResponse selectTcsStFlow(@RequestParam @ApiParam String time) {
        operationLogService.addOperationLog(this.setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.SELECT, JSON.toJSONString(time),
                "查询前一天所有结算记录"));
    ...
    public BaseResponse pageSettleAccountCheck(@RequestBody @ApiParam TradeCheckAccountDTO tradeCheckAccountDTO) {
        operationLogService.addOperationLog(this.setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.SELECT, JSON.toJSONString(tradeCheckAccountDTO),
                "分页查询机构结算对账"));
