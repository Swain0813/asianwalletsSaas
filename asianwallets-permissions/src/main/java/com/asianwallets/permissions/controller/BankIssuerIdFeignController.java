...
public class BankIssuerIdFeignController extends BaseController {
    ...
    public BaseResponse addBankIssuerId(@RequestBody @ApiParam List<BankIssuerIdDTO> bankIssuerIdDTOList) {
        operationLogService.addOperationLog(setOperationLog(this.getSysUserVO().getUsername(), AsianWalletConstant.ADD, JSON.toJSONString(bankIssuerIdDTOList),
                "添加银行机构代码映射信息"));
    ...
    public BaseResponse updateBankIssuerId(@RequestBody @ApiParam BankIssuerIdDTO bankIssuerIdDTO) {
        operationLogService.addOperationLog(setOperationLog(this.getSysUserVO().getUsername(), AsianWalletConstant.UPDATE, JSON.toJSONString(bankIssuerIdDTO),
                "修改银行机构代码映射"));
    ...
    public BaseResponse pageFindBankIssuerId(@RequestBody @ApiParam BankIssuerIdDTO bankIssuerIdDTO) {
        operationLogService.addOperationLog(setOperationLog(this.getSysUserVO().getUsername(), AsianWalletConstant.SELECT, JSON.toJSONString(bankIssuerIdDTO),
                "查询银行机构代码映射信息"));
    ...
    public BaseResponse importBankIssuerId(@RequestParam("file") @ApiParam MultipartFile file) {
        operationLogService.addOperationLog(setOperationLog(this.getSysUserVO().getUsername(), AsianWalletConstant.ADD, null,
                "导入银行机构代码映射信息"));
        return bankIssuerIdFeign.importBankIssuerId(importService.importBankIssuerId(this.getSysUserVO().getUsername(), file));
    }
