...
public class BankFeignController extends BaseController {
    ...
    public BaseResponse addBank(@RequestBody @ApiParam BankDTO bankDTO) {
        operationLogService.addOperationLog(setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.ADD, JSON.toJSONString(bankDTO),
                "增加银行"));
    ...
    public BaseResponse updateBank(@RequestBody @ApiParam BankDTO bankDTO) {
        operationLogService.addOperationLog(setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.UPDATE, JSON.toJSONString(bankDTO),
                "修改银行信息"));
    ...
    public BaseResponse pageFindBank(@RequestBody @ApiParam BankDTO bankDTO) {
        operationLogService.addOperationLog(setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.SELECT, JSON.toJSONString(bankDTO),
                "分页查询银行信息"));
    ...
    public BaseResponse importBank(@RequestParam("file") @ApiParam MultipartFile file) {
        operationLogService.addOperationLog(setOperationLog(this.getUserName().getUsername(), AsianWalletConstant.ADD, null,
                "导入银行信息"));
        return bankFeign.importBank(importService.importBank(this.getUserName().getUsername(), file));
    }
