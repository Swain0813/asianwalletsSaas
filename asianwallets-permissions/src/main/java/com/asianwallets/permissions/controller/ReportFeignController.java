...
public class ReportFeignController extends BaseController {
    ...
    public BaseResponse exportInsDailyTrade(@RequestBody @ApiParam OrdersDTO ordersDTO, HttpServletResponse response) {
        ...
        ExcelWriter writer = ExcelUtil.getBigWriter();
        try {
            List<InsDailyTradeVO> dataList = reportFeign.exportInsDailyTrade(ordersDTO);
            ServletOutputStream out = response.getOutputStream();
            if (ArrayUtil.isEmpty(dataList)) {
                //数据不存在的场合
                HashMap errorMsgMap = SpringContextUtil.getBean(CommonLanguageCacheService.class).getLanguage(getLanguage());
            ...
            ExcelUtils excelUtils = new ExcelUtils();
            excelUtils.exportExcel(dataList, InsDailyTradeVO.class, writer);
            writer.flush(out);
        } catch (Exception e) {
            log.info("==========【导出机构日交易汇总表】==========【导出机构日交易汇总表异常】", e);
            throw new BusinessException(EResultEnum.INSTITUTION_INFORMATION_EXPORT_FAILED.getCode());
        } finally {
            writer.close();
        }
