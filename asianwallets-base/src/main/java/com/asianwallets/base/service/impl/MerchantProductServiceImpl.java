           ...
           for(ProdChannelDTO prodChannelDTO : merProDTO.getProductList()){
               ...
               if(!merchantCardCode.getProductCode().contains(product.getProductCode().toString())){
                   ...
                   if(result>0){
                       try {
                           ...
                           log.error("商户码牌信息存在的场合将数据更新后存redis里：", e);
                           throw new BusinessException(EResultEnum.ERROR_REDIS_UPDATE.getCode());
                       }
                   }
               }
            ...
            if(result>0){
                try {
                    ...
                    log.error("商户码牌信息不存在新增的场合将数据新增到redis里：", e);
                    throw new BusinessException(EResultEnum.ERROR_REDIS_UPDATE.getCode());
                }
            }
        }
...
//            }
    ...
    public MerChannelVO getMerChannelInfoById(String merChannelId) {
        return merchantChannelMapper.getMerChannelInfoById(merChannelId, auditorProvider.getLanguage());
    }
